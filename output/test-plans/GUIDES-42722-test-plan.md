...
- AC-08 [Proposed]: (Integration) Given more than one metadata field is placed in the metadata page layout, each field set to show a named property such as change description or version | When Native PDF output is generated with the merged (Merge with Previous page) metadata layout | Then every metadata field shows its own named property value on the merged page and not only the change description field | Evidence: Jira GUIDES-42722; chapter.plt page layout metadata field spans.
