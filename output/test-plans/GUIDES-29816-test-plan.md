...
- AC-14 [Proposed]: (Integration) Given a PDF preset that uses the DITA-OT processing engine rather than the native engine | When PDF output is generated | Then the existing DITA-OT metadata.xml behaviour that already exposes the source map path is unchanged, and this fix changes only the native Native PDF engine path | Evidence: Jira GUIDES-29816 (DITA-OT already exposes damPath); Starling BaseExecutor nativeOutput branch.
