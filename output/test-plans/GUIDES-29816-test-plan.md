...
- AC-02 [Proposed]: (Basic) Given a Native PDF publish of a source map | When metadata.xml is generated | Then it holds the current map basic properties, including damPath and source id where available, not values from a previous map | Evidence: Starling BaseExecutor.getMetadataList damPath sourceProp; GUIDES-17324.
...
- AC-04 [Proposed]: (Integration) Given topics in the Native PDF output | When metadata.xml is generated | Then each topic keeps its own selected File (Asset) properties and no other topic or map value is used for it | Evidence: Jira GUIDES-29816.
...
- AC-09 [Proposed]: (Negative) Given a selected File (Asset) property missing on a map or topic | When Native PDF output is generated | Then generation still finishes, that property is skipped for the asset, and the remaining metadata is written as valid XML | Evidence: Jira GUIDES-29816; Starling BaseExecutor.getMetadataList property-existence guard.
...
- AC-15 [Proposed]: (Integration) Given the metadata.xml is built by shared code used by both the DITA-OT and native output engines | When another output type such as AEM Site, HTML5, or DITA-OT PDF is generated with Retain temporary files enabled | Then the change stays in scope for the Native PDF preset and the other presets are covered as shared-path regression with their retained metadata.xml unchanged | Evidence: Jira GUIDES-29816; Starling BaseExecutor.updateMetadata and getMetadataList shared by DitaOtExecutor and NodeJsExecutor.
...
- P2 [AC-15]: Action: with Retain temporary files enabled, generate a DITA-OT-based preset (AEM Site, HTML5, or DITA-OT PDF) before and after the change and open each retained metadata.xml. Expected: the shared metadata.xml content is unchanged apart from the intended File (Asset) properties behaviour, confirming the shared BaseExecutor path did not regress other outputs.
...
- Re-run other output types that use the shared BaseExecutor metadata path (AEM Site, HTML5, DITA-OT PDF) with Retain temporary files enabled and confirm their retained metadata.xml is unaffected, because BaseExecutor.updateMetadata and getMetadataList are shared by DitaOtExecutor and NodeJsExecutor and a change there could regress outputs beyond Native PDF.
