...
6. Terminal-event discipline (one user-visible conversation per UAC request): child-session lifecycle events - idle/completion notices, archive confirmations, cleanup acknowledgements - are INTERNAL TERMINAL EVENTS, never new work items. They must not trigger orchestration (no new delegation, no resume pass, no UAC state change) and must never recursively trigger another archive/response cycle; the parent performs at most one internal completion transition per child execution, and an archived leaf session is never messaged or re-entered. Normal research results and error results still arrive exactly once through the return-handoff message - that channel is unaffected. After the final render (or a hard stop), archive the run's leaf sessions in ONE batch; before that point keep them alive only while their result is uncollected or the run may need a resend.
7. Debug mode is the only exception: when the user explicitly asks for diagnostics/audit, or `AGENT_RESEARCH_DEBUG=1` is set in the environment, the parent may surface leaf execution IDs, session links, receipts, and logs, and skips the batch archive. Normal UAC generation never shows them.
