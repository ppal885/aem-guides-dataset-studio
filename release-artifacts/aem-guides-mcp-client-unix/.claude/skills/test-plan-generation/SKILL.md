...
- **Relevance check before delivery:** for every AC, name the affected user and the ticket sentence, attachment or product decision it serves. An AC whose screen is used by a different user than the one in the complaint (for example a creator's list when the complaint is about the assignee's list), or that rests only on your own inference, is removed or becomes a TBD. Irrelevant ACs cost reviewers as much as missing ones. Enforced by `uac_release_runner.orphan_ac_problems`: every AC must be driven by a ticket sentence or attachment in SOURCE_COVERAGE, name a requested surface, be a still-works check on an inventoried surface, or carry a TBD; an AC whose only source is QE reasoning needs a TBD. Rules apply at generation time only, so after editing a UAC by hand run `python scripts/uac_release/uac_release_runner.py --check-dir <folder>` and post only when it passes.
