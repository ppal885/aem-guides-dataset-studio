# How to verify — checks and failure injection reference

> **Plain English:** This file supports **how to verify / how to check / pass if** in test plans. Internal name: test oracles. In the **plan body**, never write "oracle" — say what to check (HTTP code, CRX property, log line, UI state).

A scenario is not review-ready unless it has concrete verification steps. `No error`, `works correctly`, and `verify behavior` are not valid expected results by themselves.
