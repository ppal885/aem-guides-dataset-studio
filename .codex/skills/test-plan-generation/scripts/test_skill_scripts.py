...
def test_uac_fidelity_reference() -> None:
    ...
    for marker in (
        ...
        "data-rhwidget='TopicHeader'",
        "data-rhwidget='TopicTitle'",
        "/html/body/h1[1]",
        "## Accepted Reference: GUIDES-50143 Baseline Label Autocomplete Input Parity",
        "containing/starting with",
        "The phrase `containing/starting with` is ambiguous",
        "Real time` is a functional responsiveness phrase",
        "## Accepted Reference: GUIDES-49144 Table-Cell Inline Whitespace Preservation",
        "root_cause_source=jira_comment_engineering_rca",
        "GUIDES-28188 is a candidate same-mechanism regression reference",
        "Which exact DITA elements are meant by `text-forbidden tags`?",
        "AC-07 [Proposed]: (Publishing Regression)",
        "## Accepted Reference: GUIDES-48587 Alphanumeric User-Dictionary Spellcheck",
        "numeric characters at its starting or ending boundary",
        "root_cause_source=missing",
        "automation remains blocked until the character/position matrix is approved",
        "/apps/fmdita/config/user_dictionary.txt",
        "Candidate implementation flow `SpellCheckFeature` to `AEMSpellChecker`",
        ...
        "uac_source_origin=user_provided_final_uac",
        "Only the exact profile-application contract is confirmed",
        "AC-01 [Confirmed]: (Publishing)",
        "Coated_Fogra39L_VIGC_300.icc",
        "two independent failure mechanisms",
        "a successful job without profile application does not satisfy this AC",
        "Visual comparison is supporting evidence only",
        "GUIDES-25967`, `GUIDES-25017`, and `GUIDES-14741",
        "## Accepted Reference: GUIDES-48450 DITA Element Predicate Request Serialization",
        "uac_source_origin=jira_acceptance_field_and_user_transcription",
        "211_group.1_property.value=difference",
        "empty skeleton",
        "root_cause_source=missing",
        "AC-05 [Confirmed]: (Tokenization)",
        "Guides Editor Repository search is a control",
        "## Accepted User-Provided Reference: Per-Topic Review Completion",
        "primary_jira_key=missing",
        "figma_source_status=degraded_reauthentication_required",
        "Complete review task",
        "does not track whether a topic has merely been opened",
        "Any topic version change clears all previous completion status",
        "does not authorize automatic task completion",
        "backend reviewer authorization",
        "No Confirmed AC is justified for notification payloads",
        "Incident workload observations are not performance SLAs",
    ):
