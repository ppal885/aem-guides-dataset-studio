...
- Minimum evidence manifest fields are `"rag_tool": "ask_dita_expert"`, three exact `rag_probes`, `"jira_history_tool": "search_jira_history"`, `"indexed_history_run": true`, two `jira_history_queries` (same-customer and cross-customer), and an `evidence_graph` record containing requested/tool/status/influence_mode/used_for_plan/generation/query/runtime/cache/path/leaf provenance or an explicit degraded reason.
...
- **Acceptance Criteria**: Write `AC-## [Confirmed]` for Jira-approved behavior and `AC-## [Proposed]` for criteria derived from requirements, incidents, or identified gaps. Draft each one as a self-contained contract with an explicit precondition/input, trigger, observable outcome, scope boundary, and verification oracle. Prefer Given/When/Then, name exact properties/fields/enums and expected values, and avoid hidden assumptions. Every AC mapped to a P0 or P1 scenario must end with ` | Evidence: <underlying Jira, URL/chunk, DITA source, Figma node, attachment, or inspected code citation>`; a graph path ID alone is invalid. Never write a terse label or generic `Verify...` step. Keep sign-off-critical unknowns in `Open Questions` and add a Draft blocker only when they prevent UAC readiness.
