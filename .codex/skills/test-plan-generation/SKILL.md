...
- When the Jira, supplied incident, or indexed issue identifies a customer, retrieve its `customer_jira_profile` aggregate chunks or customer-profile API record. Use the profile only to discover frequently represented or affected components, workflows, outputs, DITA entities, failure areas, automation signals, and candidate historical keys.
...
- **Known Jira Bugs / Past Similar Tickets**: Start with an `Observed Customer Jira Profile:` bullet when customer profile evidence exists. Report profile version and approval status, distinct-key issue count, classification coverage, frequently represented or affected issue/problem types, product areas, components, workflows, outputs, DITA/content-data patterns, automation patterns, and representative candidate keys. State that draft/rejected profiles are unreviewed aggregate guidance and that even approved profiles remain aggregate context rather than direct product-behavior proof. Then list up to five directly validated Jira keys covering relevant open known bugs and resolved historical bugs. Include status/resolution, similarity, RCA or behavior lesson when available, affected/fix version, reusable test evidence, concrete impact on scenarios or regression, and the narrow Jira/JQL and indexed-history search status. Mark every unavailable field explicitly instead of inferring or omitting it. Never use the aggregate profile itself as a test oracle.
