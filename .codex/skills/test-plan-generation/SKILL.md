...
- Minimum dual-source manifest fields are `"rag_tool": "ask_dita_expert"`, three exact `rag_probes`, `"jira_history_tool": "search_jira_history"`, `"indexed_history_run": true`, and two `jira_history_queries`: one `same_customer` record with query/component/customer and one `cross_customer` record with query/component and no customer.
...
- Treat the `component` argument to `search_jira_history` as a strict Chroma filter over normalized scalar `component_primary` metadata. If a broader second pass is needed, intentionally omit the component and label any retained result as cross-component instead of presenting it as a same-component match.
