...
- Minimum manifest fields include the complete `evidence_preflight` contract from `references/evidence-preflight.md`, `"rag_tool": "ask_dita_expert"`, three exact `rag_probes`, `"jira_history_tool": "search_jira_history"`, `"indexed_history_run": true`, and two `jira_history_queries`: one `same_customer` record with query/component/customer and one `cross_customer` record with query/component and no customer.
