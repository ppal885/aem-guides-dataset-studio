...
- Querying the indexed `jira_qa` history of already-fixed tickets is a mandatory Phase-4 step, not optional, and must run before writing Known Jira Bugs; ask it specifically for the issue's customer/label footprint and problem area (the indexed corpus holds past fixed tickets by area and customer, not test data or a live customer inventory - do not ask it to invent folder paths or fixtures). Record the fact that it was run in the evidence manifest (`indexed_history_run`), and in the search-status bullet state which surfaced tickets were kept as same-mechanism and which were excluded as different-mechanism usage-area matches. Every ticket it surfaces is still mutable history: validate status/resolution/fix-version live before citing, and never present an indexed hit as current truth.
...
- **Acceptance Criteria**: Write `AC-## [Confirmed]` for Jira-approved behavior and `AC-## [Proposed]` for criteria derived from requirements, incidents, or identified gaps. Draft each one the way a Principal Manual QA engineer specifies a product contract: an explicit precondition or input, the trigger, and the observable expected outcome, with the scope boundary stated (what is included and what is explicitly excluded) and the verification oracle implied (which property, field, column, API, or log confirms it). Prefer a Given/When/Then shape in a single sentence, name exact properties/fields/enums and their expected values, and make each criterion pass or fail on its own with no hidden assumption about defaults, ordering, counts, or excluded cases. Never write an acceptance criterion as a terse label or a generic `Verify...` test step. If a sign-off-critical decision is unknown, keep it in `Open Questions` and add a Draft blocker only when it prevents UAC readiness.
