...
- **Known Jira Bugs / Past Similar Tickets**: Start with an `Observed Customer Jira Profile:` bullet when customer profile evidence exists. Summarize distinct-key issue count, frequently represented or affected components/workflows/outputs/entities, automation patterns, representative candidate keys, and the aggregate-evidence limitation. Then list up to five directly validated Jira keys covering relevant open known bugs and resolved historical bugs. Include status/resolution, similarity, RCA or behavior lesson when available, affected/fix version, reusable test evidence, concrete impact on scenarios or regression, and the narrow Jira/JQL and indexed-history search status. Mark every unavailable field explicitly instead of inferring or omitting it. Never use the aggregate profile itself as a test oracle.
