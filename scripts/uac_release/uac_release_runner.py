...
One scheduled run:
  ...
  5. posts a draft comment with the plan attached and adds the draft label.
...
When finished, write exactly two files:
...
def draft_comment(field_body: str, plan_name: str) -> str:
    return (
...
def process_ticket(key: str, config: dict, jira, logger, dry_run: bool) -> str:
    ...
    for name in (common.UAC_FILE, common.PLAN_FILE):
    ...
    if problems:
        ...
        return "FAILED"
    field_body = common.import_skill_module("jira_safe_text").jira_field_body(
        (ticket_dir / common.UAC_FILE).read_text(encoding="utf-8"))
    ...
    comment_id = jira.add_comment(key, draft_comment(field_body, plan_copy.name))
