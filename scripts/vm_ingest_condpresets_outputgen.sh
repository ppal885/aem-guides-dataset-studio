#!/usr/bin/env bash
# Paste into the VM terminal. Writes the ingestion script into backend/ and runs it there.
set -e
cd ~/aem-guides-dataset-studio/backend

cat > ingest_condpresets_outputgen.py << 'PYEOF'
...
python3 ingest_condpresets_outputgen.py
