...
SCHEMA_VERSION = "aem-guides-concurrency-race-v1"
...
def validate_concurrency_race_analysis(block, *, ac_ids=None, open_question_ids=None):
    ...
    known_open_question_ids = None if open_question_ids is None else set(open_question_ids)
    problems = []
    ...
    if block.get("schema_version") != SCHEMA_VERSION:
        problems.append(f"concurrency_race_analysis.schema_version must be {SCHEMA_VERSION}")
    active = block.get("active")
    if not isinstance(active, bool):
        return problems + ["concurrency_race_analysis.active must be true or false"]

    if not active:
        if not str(block.get("reason", "")).strip():
            problems.append("inactive concurrency_race_analysis requires a non-empty reason")
        return problems
    ...
    for entry in patterns:
        ...
        if disposition == "COVERED_BY_AC":
            ...
            if not oq_ref or (
                known_open_question_ids is not None and oq_ref not in known_open_question_ids
            ):
