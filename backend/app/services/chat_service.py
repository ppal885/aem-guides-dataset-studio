...
def _build_dita_ot_preprocess_runtime_fallback_response(user_content: str) -> str:
    ...
    if (
        ...
        return "\n".join([
            ...
            "The original authored source XML can therefore differ from the generated effective resource, the conref-resolved effective processed content, and the final filtered output. If reused content includes an `@id` or an `xref`, verify how that content reference behaves after relocation into the consuming topic.",
            "",
            "## Example source",
            "```xml",
            "<map>",
            ...
            "",
            "## Expected result",
            "You should be able to say exactly which stage changed the content: `copy-to` changed resource identity and may create two effective topic resources from one authored topic, `conref` changed effective processed content, or `profile` removed the content for the selected PDF/filtering context.",
            "",
            "## Sources",
            f"- {copy_to_url}",
            f"- {conref_url}",
    ...
    if re.search(
        ...
        return "\n".join([
            ...
            "- A `resource-only` topic can still be used as a reuse source if it is reachable for processing; resource-only controls normal output/navigation and TOC participation, not whether the file can be a reuse target.",
...
def _build_direct_tool_response(name: str, result: dict[str, Any]) -> str:
    ...
    if name == "generate_dita_ot_pdf":
        ...
        pdf_urls = [str(item) for item in (result.get("pdf_download_urls") or []) if str(item).strip()]
