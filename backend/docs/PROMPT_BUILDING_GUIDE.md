...
Chat uses `chat_system.json` with sections: role_and_tone, capabilities, tool_rules, jira_rule, grounding, examples, formatting.
...
5. **Grounding** – Base answers on RAG context; don't hallucinate
6. **Examples** – Few-shot examples for Jira paste and natural language
...
| Frontend `ChatPage` | `context` passed to `sendMessage` (e.g. `source_page`, `issue_summary`) |
