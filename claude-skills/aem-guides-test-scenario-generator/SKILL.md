...
description: >
  Generate DALP-ready, bug-discovery-first AEM Guides QA test plans from Jira using Claude Code
  as the orchestrator. Use when a QA/dev asks for AEM Guides test plans, regression strategy,
  impact/risk analysis, evidence-grounded UACs, detailed scenarios, automation mapping, or DITA
  construct coverage. Requires Adobe Jira MCP, local product/test repos when available, and the
  existing VM RAG/MCP evidence source; do not create duplicate Jira clients, RAG systems, vector
  databases, repository services, pipeline applications, or duplicate Skills.
---
